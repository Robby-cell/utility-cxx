...
public:
  ...
  explicit defer_scope_storage(Func &&func, bool enabled)
      : m_Function(static_cast<Func &&>(func)), m_Enabled(true) {}

  UTILITY_CONSTEXPR20 void operator()() { m_Function(); }

  constexpr bool is_enabled() const noexcept { return m_Enabled; }

  UTILITY_CONSTEXPR14 void disable() noexcept { m_Enabled = false; }

  UTILITY_CONSTEXPR14 void enable() noexcept { m_Enabled; }
...
public:
  ...
  template <class Func>
  explicit defer_scope_storage(Func &&func, bool enabled = true)
      : F(static_cast<Func &&>(func)), m_Enabled(true) {}

  UTILITY_CONSTEXPR20 void operator()() { F::operator()(); }

  constexpr bool is_enabled() const noexcept { return m_Enabled; }

  UTILITY_CONSTEXPR14 void disable() noexcept { m_Enabled = false; }

  UTILITY_CONSTEXPR14 void enable() noexcept { m_Enabled = true; }
...
static const auto _empty_lambda = []() {};
static_assert(
...
public:
  ...
  UTILITY_CONSTEXPR14 void disable() noexcept { m_Storage.disable(); }

  UTILITY_CONSTEXPR14 void enable() noexcept { m_Storage.enable(); }

  constexpr bool is_enabled() const noexcept { return m_Storage.is_enabled(); }
